...
from transformers.activations import get_activation, ACT2FN, gelu
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.apply_lora:
            self.query = lora.Linear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, lora_norm=False)
        else:
        ...
        if config.apply_lora:
            self.value = lora.Linear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha,lora_dropout=config.lora_dropout, lora_norm=False)
        else:
        ...
        if config.apply_prefix_tuning:
            self.prefix_k = nn.Parameter(torch.zeros((config.hidden_size, config.prefix_size)))
            self.prefix_v = nn.Parameter(torch.zeros((config.hidden_size, config.prefix_size)))
...
class RobertaLayer(nn.Module):
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        if self.apply_parallel_adapter:
            layer_output = self.LayerNorm(self.parallel_adapter(attention_output, 0.1 * layer_output))
        return layer_output
